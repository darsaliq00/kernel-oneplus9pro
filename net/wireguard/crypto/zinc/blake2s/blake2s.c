		      ...
		      const void *key, const size_t keylen)
{
	...
	blake2s_update_wg(state, block, BLAKE2S_BLOCK_SIZE);
...
void blake2s_update_wg(struct blake2s_state *state, const u8 *in, size_t inlen)
{
...
void blake2s_final_wg(struct blake2s_state *state, u8 *out)
{
		  ...
		  const size_t inlen, const size_t keylen)
{
	...
	if (keylen > BLAKE2S_BLOCK_SIZE) {
		...
		blake2s_update_wg(&state, key, keylen);
		blake2s_final_wg(&state, x_key);
	} else
	...
	blake2s_init(&state, BLAKE2S_HASH_SIZE);
	blake2s_update_wg(&state, x_key, BLAKE2S_BLOCK_SIZE);
	blake2s_update_wg(&state, in, inlen);
	blake2s_final_wg(&state, i_hash);
	...
	blake2s_init(&state, BLAKE2S_HASH_SIZE);
	blake2s_update_wg(&state, x_key, BLAKE2S_BLOCK_SIZE);
	blake2s_update_wg(&state, i_hash, BLAKE2S_HASH_SIZE);
	blake2s_final_wg(&state, i_hash);
